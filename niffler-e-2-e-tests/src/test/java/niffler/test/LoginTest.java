...
import niffler.jupiter.UsersListExtension;
...
@ExtendWith({ScreenshotExtension.class, UsersListExtension.class})
public class LoginTest extends BaseTest {

    @AllureId("1")
    @Test
    void mainPageShouldBeDisplayedAfterSuccessLogin(@User(userType = ADMIN) UserModel userAdmin,
                                                    @User(userType = COMMON) UserModel userCommon) {
        List<UserModel> users = List.of(userAdmin, userCommon);
        users.forEach(data -> {
    ...
    @AllureId("3")
    @Test
    void mainPageShouldBeDisplayedAfterSuccessLogin1(@User(userType = ADMIN) UserModel user) {
    ...
    @AllureId("4")
    @Test
    void mainPageShouldBeDisplayedAfterSuccessLogin4(@User(userType = ADMIN) UserModel user) {
